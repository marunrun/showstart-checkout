...
namespace checkout.Helper
{
    ...
    class RequestUtil
    {
        ...
        public async static Task<string> post(RequestQo request, object param,  string sessionId = null)
        {
            HttpContent content = new StringContent(JsonSerializer.Serialize(buildeRquest(request, param)));
            content.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue("application/json");
            content.Headers.Add("CUSUT", UserService.getSign());
        ...
        public static Dictionary<string, string> buildeRquest(RequestQo request, Object param, DateTime dateTime)
        {
            ...
            var dataKey = UserService.getAESKey();
            var aruKey =PublicData.ARU_KEY;
            if (request.bol)
            {
                dataKey = aruKey;
            }
